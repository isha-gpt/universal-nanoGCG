...
from nanogcg.configs import GCGConfig, ProbeSamplingConfig, GCGResult
...
class GCG:
    def __init__(
        ...
        self.dual_model = self.config.dual_model
        self.dual_embedding_layer = self.dual_model.get_input_embeddings() if self.dual_model is not None else None
    ...
    def _compute_candidates_loss_probe_sampling(
        ...
        draft_probe_losses = draft_losses[probe_idxs.cpu()]
        rank_correlation = spearmanr(
            probe_losses.cpu().float().numpy(),
            draft_probe_losses.cpu().float().numpy(),
        ).correlation
    ...
    def _compute_candidates_loss_original(
        ...
        device = self.model.device  # primary device (e.g. cuda:0)
        
        # Clone and move input embeddings and target_ids to the primary device
        input_embeds = input_embeds.clone().detach().to(device)
        self.target_ids = self.target_ids.clone().detach().to(device)
        ...
        for i in range(0, input_embeds.shape[0], search_batch_size):
            with torch.no_grad():
                # Clone each batch slice to ensure a fresh copy on the correct device
                input_embeds_batch = input_embeds[i:i + search_batch_size].clone().detach().to(device)
                current_batch_size = input_embeds_batch.shape[0]

                if self.prefix_cache:
                    if not prefix_cache_batch or current_batch_size != search_batch_size:
                        prefix_cache_batch = [
                            [x.clone().detach().to(device).expand(current_batch_size, -1, -1, -1) 
                            for x in self.prefix_cache[j]]
                ...
                logits = outputs.logits.clone().detach().to(device)
                
                tmp = input_embeds.shape[1] - self.target_ids.shape[1]
                shift_logits = logits[..., tmp-1:-1, :].clone().detach().contiguous().to(device)
                shift_labels = self.target_ids.clone().detach().repeat(current_batch_size, 1).to(device)
                
                if self.config.use_mellowmax:
                    label_logits = torch.gather(shift_logits, -1, shift_labels.unsqueeze(-1)).squeeze(-1).clone().detach().to(device)
                    loss = mellowmax(-label_logits, alpha=self.config.mellowmax_alpha, dim=-1).clone().detach().to(device)
                else:
                ...
                loss = loss.view(current_batch_size, -1).mean(dim=-1).clone().detach().to(device)
        ...
        return torch.cat(all_loss, dim=0).clone().detach().to(device)
...
def run(
    ...
    if isinstance(messages, list):
        # Delegate to the multi-prompt method via UniversalGCG.
        from submodules.nanoGCG.nanogcg.universal_gcg import UniversalGCG
        universal = UniversalGCG(model, tokenizer, config, logger=logger)
        result = universal.run_multi_prompt(messages, target)
    else:
